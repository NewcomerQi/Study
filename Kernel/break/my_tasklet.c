...
// 中断下半部工作
static void work_handler(unsigned long data)
{
	// 禁止本地处理器的软中断和tasklet的下半部处理
	local_bh_disable();
	printk("Ths is work_handler func\n");
	printk("key_code: %ld\n", data);
	send_signal(SIGUSR1, data);
	// 激活本地处理器的软中断和tasklet的下半部处理
	local_bh_enable();
}
...
static int __init mydev_driver_init(void)
{
	...
	// 初始化tasklet
	tasklet_init(&my_tasklet, work_handler, 0);
